package eu.esrocos.kul.robot.generator.common.gr

import eu.esrocos.kul.robot.kinDsl.Robot
import eu.esrocos.kul.robot.kinDsl.AbstractLink
import eu.esrocos.kul.robot.kinDsl.RotoTrasl
import eu.esrocos.kul.robot.kinDsl.Joint

import eu.esrocos.kul.robot.generator.common.gr.AttachedFrame.FrameRole
import eu.esrocos.kul.robot.generator.common.gr.RobotFrameUtils.FramesGraphEdge
import eu.esrocos.kul.robot.generator.common.TreeUtils
import eu.esrocos.kul.robot.generator.common.Vector3D
...
class RobotPoseUtils
{
    public new(Robot rob) {
        this(rob, new TreeUtils(rob))
    }

    public new(Robot rob, TreeUtils treeutils) {
        robot = rob
        tree  = treeutils
    }
    ...
    static class ConstantPose extends RelativePose
    {
        public new(AttachedFrame tgt, AttachedFrame ref, RotoTrasl params)
        {
            ...
            if( ref.role == FrameRole.link ) {
                ref_R_target = NumericUtils.original_X_rotated(
                                       utils.asFloat(params.rotation.x),
                                       utils.asFloat(params.rotation.y),
                                       utils.asFloat(params.rotation.z) )
            } else {
                ref_R_target = NumericUtils.rotated_X_original(
                                              utils.asFloat(params.rotation.x),
                                              utils.asFloat(params.rotation.y),
                                              utils.asFloat(params.rotation.z))
                val newvec = NumericUtils.matrix3x3Mult(ref_R_target, refToTarget)
